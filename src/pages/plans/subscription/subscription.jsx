	...
	const fetchPrices = () => {
		fetch(`https://colonpax.com/api/products.php`)
			...
			.then((res) => {
				...
				setCampaignIds(Object.keys(res.message.data).slice(0, 2).reverse());
				console.log(Object.keys(res.message.data).slice(0, 2).reverse(), option1Products, option2Products);
			});
	};
			...
			<div className="container">
				<div className="subscription row g-5">
					{data.map((item, index) => {
						...
						const price = products[value][index] ? parseFloat(products[value][index].price) : 0;
							...
							<div className="col-12 col-lg-4" key={id}>
								<div className={active ? 'subscription-item active' : 'subscription-item'}>
									...
									<div className="form-check mb-4">
										<input
											className={radioChecked ? 'form-check-input checked' : 'form-check-input '}
											type="radio"
											name={`flexRadioDefault1${id}`}
										...
										onClick={() =>
											dispatch(
												subscriptionPopupOpen({
													id: id,
													plan: `${dose} ${dose > 1 ? 'Dosen' : 'Dose'}  / ${dose} Monatsvorrat`,
													qty: dose,
													type: value === 1 ? 'Einmaliger Kauf' : 'Abonnieren & Sparen',
													price,
													subtotal: price + price * 0.07,
													campaignId: campaignIds.length > 0 ? campaignIds[value] : '2',
													discount: showSparan === -1 || value === showSparan ? sparanNum : 0,
												})
											)
										}
									>
